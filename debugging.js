...
function rootPath(req, res) {
    ...
    try {
        if (ENV_MODE === MODE.PROD) throw new ErrorMessage.DevModeException('Not Dev Mode', '/')
        res.send({
...
async function viewCookie(req, res) {
    ...
    try {
        if (ENV_MODE === MODE.PROD)
            throw new ErrorMessage.DevModeException('Not Dev Mode', 'viewCookie')
        res.send({
...
async function viewSession(req, res) {
    ...
    try {
        if (ENV_MODE === MODE.PROD)
            throw new ErrorMessage.DevModeException('Not Dev Mode', 'viewSession')
        ...
        if (!checkAuthSession)
            throw new ErrorMessage.UnAuthorized('No Session Ihfo on SessionStorage')
